...
class Registro extends Component {
	...
	nextForm() {
		...
		if(this.state.isfullEntities.isfull) {
			...
			if(this.state.isfullProject.isfull) {
				...
				if(this.state.isfullCurrent.isfull) {
					this.setState({mensaje: " ", 
						title: "Autodiagnóstico estado al cual se quiere llegar y la necesidad identificada:", 
						buttonState: "Finalizar", 
						next: <InformationDesiredState getIsFullDesired={this.getIsFullDesired.bind(this)}/>, 
						anterior: <InformationCurrentState getIsFullCurrent={this.getIsFullCurrent.bind(this)}/>})
					...
					if(this.state.isfullDesired.isfull) {
						...
						swal({
						  type: 'success',
						  title: 'Buen Trabajo',
						  text: 'Has completado este formulario y has finalizado el registro!!!',
						})
					}
				} 
			} 
  	...
  	if(this.state.hiden) {
  		...
  		buttonview = <Link to="/Home"><button>SALIR</button></Link>;
  	}
    return (
    	<center>
    	<div className="App">
